...
public class GitRevList {
    private static final String[] FIRST_COMMIT_CMD = new String[]{"git", "rev-list", "--max-parents=0", "HEAD"};
                ...
                cmdRes -> {
                    if (cmdRes == null || cmdRes.size() != 1) {
                        throw new IllegalStateException(String.format("Error response from %1$s:\n%2$s",
                                StringUtils.join(FIRST_COMMIT_CMD, " "), cmdRes == null ? "<null>" : StringUtils.join(cmdRes, "\n")));
                    }
                    return cmdRes.get(0);
                });
    }
